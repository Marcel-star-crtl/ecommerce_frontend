...
const SkinCare = () => {
  ...
  useEffect(() => {
    if (skinCareCategory && skinCareCategory._id) {
      console.log("SkinCare: Dispatching fetchProductsByCategory for Skin Care category:", skinCareCategory._id);
  ...
  }, [skinCareCategory, dispatch, page, pageSize]);
  ...
  useEffect(() => {
    if (status === "failed") {
      console.error("SkinCare Error:", error);
    }
  }, [status, error]);
