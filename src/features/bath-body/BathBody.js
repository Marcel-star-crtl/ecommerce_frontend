...
const BathBody = () => {
  ...
  const [bathBodyCategory, setBathBodyCategory] = useState(null);
  ...
  useEffect(() => {
    if (allCategories && allCategories.length > 0) {
      const bathBodyCategory = allCategories.find(
        category => category.title.toLowerCase().includes('bath') && category.title.toLowerCase().includes('body') ||
                   category.title.toLowerCase().includes('bath & body') ||
                   category.title.toLowerCase().includes('bath and body') ||
                   category.title.toLowerCase() === 'bath body'
      );
      
      if (bathBodyCategory) {
        setBathBodyCategory(bathBodyCategory);
      }
    }
  }, [allCategories]);

  useEffect(() => {
    if (bathBodyCategory && bathBodyCategory._id) {
      console.log("BathBody: Dispatching fetchProductsByCategory for Bath & Body category:", bathBodyCategory._id);
      dispatch(fetchProductsByCategory({ 
        categoryId: bathBodyCategory._id, 
        pageSize, 
        page 
      }));
    }
  }, [bathBodyCategory, dispatch, page, pageSize]);
  ...
  useEffect(() => {
    if (status === "failed") {
      console.error("BathBody Error:", error);
    }
  }, [status, error]);

  const isLoading = allCategoriesStatus === 'loading' || 
                   (allCategoriesStatus === 'succeeded' && !bathBodyCategory) ||
                   status === 'loading';

  const hasError = allCategoriesStatus === 'failed' || 
                  (allCategoriesStatus === 'succeeded' && allCategories.length > 0 && !bathBodyCategory) ||
                  status === 'failed';
      ...
      <MDBRow>
        {isLoading ? (
          ...
          status === "succeeded" && (
            ...
            >
              <h3 className="text-dark fw-bold mb-3 text-center">
                No Bath & Body products available yet
              </h3>
              <span className="text-muted mb-4 text-center fs-6">
